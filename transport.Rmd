...
### Bus Access
```{r mapBus, fig.align="center", fig.cap="Map of Census block groups with the highest concentrations of one or more priority populations AND no bus stops within 1 mile."}
...
### Headways
```{r mapHwy, fig.align="center", fig.cap="Map of Census block groups with the highest concentrations of one or more priority populations AND highest average bus headways in the state."}
# create color palette
...
### Walkability
```{r mapWalk, fig.align="center", fig.cap="Map of Census block groups with the highest concentrations of one or more priority populations AND least walkable scores."}
# create color palette
...
### Cost Burden
```{r mapTcost, fig.align="center", fig.cap="Map of Census block groups with the highest concentrations of one or more priority populations AND highest transportation cost burden as a percentage of household income."}
# create color palette
...
busAccesbyTown <- ma_busBuff400m_sf %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(st_transform(ma_towns, st_crs(ma_busBuff400m_sf))) %>% 
...
town_busAccess_pct <- ma_towns_sf %>% 
  as.data.frame() %>% 
...
busAccesbyHouse <- ma_busBuff400m_sf %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(
    st_make_valid(
    st_transform(house_districts,st_crs(ma_busBuff400m_sf))
    )
    ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
house_names_pops <- ma_blkgrps_sf %>% 
  st_transform(.,st_crs(ma_busBuff400m_sf)) %>% 
  select(totalpopE) %>% 
...
# calculate bus access pops per district
busAccesbySenate <- ma_busBuff400m_sf %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(
    st_make_valid(
    st_transform(senate_districts,st_crs(ma_busBuff400m_sf))
    )
    ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
* Pct Bus Access. Average percentage of the population within walking distance of a bus stop.

* Avg Headway. The average headway (i.e., time between bus arrivals) for Census block groups in that jurisdiction. 
