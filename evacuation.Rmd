...
### Flood
```{r mapNFHZA, fig.align="center", fig.cap="Map of high concentrations of priority populations living in FEMA flood zones by Census block group."}
...
### Hurricane
```{r mapHEVAC, fig.align="center", fig.cap="Map of high concentrations of priority populations living in hurricane evacuation zones by Census block group."}
PopupEJ <- paste0(EJbgs$NAME, "<br/>",
                  "<b>Town:</b> ", EJbgs$TOWN, "<br/>",
                  "<b>State House District:</b> ", EJbgs$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", EJbgs$SEN_DIST, "<br/>",
...
leaflet() %>% 
  ...
  addPolygons(data = ma_blkgrps_sf_HEVAC, 
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE)) %>% 
  addLegend(colors = "red",
            labels = "Hurricane Evacuation Risk",
            position = "bottomleft") %>% 
  setView(lng = -71.7, 42.1, zoom = 8) %>% 
  # addMiniMap() %>% 
  addScaleBar(position = "bottomright") %>% 
