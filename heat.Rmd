...
Heat is the leading cause of weather-related death in the U.S. Warmer air temperatures can also exacerbate air pollution, especially ground-level ozone. As the climate warms, heat presents both a direct and indirect threat to health and quality of life. These interactive figures identify communities across Massachusetts that are most overburdened and most vulnerable to three measures of ambient temperature: Day-Night average land surface temperatures (LST), the Urban Heat Island effect, and elevated nighttime LST.
...
### Day-Night Avg
```{r mapDayNight, fig.align="center", fig.cap="Map of highest day-night average land surface temperatures (LST) and priority populations by Census block group."}
...
leaflet() %>% 
  ...
  addPolygons(data = ma_blkgrps_sf_DayNight, 
              ...
              highlight = highlightOptions(
                ...
                fillOpacity = 0.7,
                bringToFront = TRUE)) %>% 
  addLegend(title = "Day-Night Avg LST (°F)", 
            pal = pal,
...
### Urban Heat Island
```{r mapDayNightUHI, fig.align="center", fig.cap="Map of highest day-night average urban heat island (UHI) effects and priority populations by Census block group."}
# create color palette
pal <- colorQuantile(
  palette = "YlOrRd",
...
leaflet() %>% 
  ...
  addPolygons(data = ma_blkgrps_sf_UHI24h, 
              ...
              highlight = highlightOptions(
                ...
                fillOpacity = 0.7,
                bringToFront = TRUE)) %>% 
  addLegend(title = "Day-Night UHI (°F)", 
            pal = pal,
...
### Night
```{r mapNight, fig.align="center", fig.cap="Map of highest nighttime average land surface temperatures (LST) and priority populations by Census block group."}
# create color palette
pal <- colorQuantile(
  palette = "YlOrRd",
...
PopupHouse <- paste0("Massachusetts state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGs80thDayNight,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing nighttime average land surface temperatures at the 80th percentile - ", round(min(ma_blkgrps_sf_Night$meanNightLST),1),"° - or higher.", "<br/>",
...
PopupSenate <- paste0("Massachusetts state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGs80thDayNight,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing nighttime average land surface temperatures at the 80th percentile - ", round(min(ma_blkgrps_sf_Night$meanNightLST),1),"° - or higher.", "<br/>",
...
leaflet() %>% 
  ...
  addPolygons(data = ma_blkgrps_sf_Night, 
              ...
              highlight = highlightOptions(
                ...
                fillOpacity = 0.7,
                bringToFront = TRUE)) %>% 
  addLegend(title = "Night Avg LST (°F)", 
            pal = pal,
...
LST24hrPop %>% 
  ...
  hc_yAxis(title = list(text = "Population-weighted temperature (°F)"),
           labels = list(format = "{value}°"), 
           floor = 72, ceiling = 85) %>% 
  hc_xAxis(title = NULL, type = "category", labels = list(step = 1)) %>% 
  hc_tooltip(pointFormat = "{series.name}: {point.y}°") %>% 
  hc_title(text = "Population−Weighted Temperature Exposure to Day−Night Average LST (°F)", useHTML = TRUE)
```
...
UHI24hrPop %>% 
  ...
  hchart(., "bar", hcaes(x = group_index, y = UHI24hrwMean, 
                         group = Status, name = Group), 
                         color = c("#F7A35C", "#7CB5EC", "#000000")) %>% 
  hc_yAxis(title = list(text = "Population-weighted UHI effect (°F)"),
           labels = list(format = "+{value}°"),
           ceiling = 12) %>% 
  hc_xAxis(title = NULL, type = "category", labels = list(step = 1)) %>% 
  hc_tooltip(pointFormat = "{series.name}: +{point.y}°") %>% 
  hc_title(text = "Population−Weighted Temperature Exposure to Day−Night Average Urban Heat Island effect (°F)", useHTML = TRUE)
```
...
LSTNightPop %>% 
  ...
  hc_xAxis(title = NULL, type = "category", labels = list(step = 1)) %>% 
  hc_tooltip(pointFormat = "{series.name}: {point.y}°") %>% 
  hc_title(text = "Population−Weighted Temperature Exposure to Nighttime Average LST (°F)", useHTML = TRUE)
```
...
For example, limited English speaking households, as identified by state environmental justice policy (i.e., 'MA Limited English HH'), reside in Census block groups that experience population-weighted average day-night land surface temperatures of over `r round(LST24hrPop %>% filter(Group == "MA Limited English HH") %>% select(LST24hrwMean) %>% pull(),1)`°F. Compare this to the state average (i.e., `Total Pop`) of `r round(LST24hrPop %>% filter(Group == "Total Pop") %>% select(LST24hrwMean) %>% pull(),1)`°F. With regard to the Urban Heat Island (UHI) effect, all groups experience average day-night temperatures above the rural background. limited English speaking households, as identified by state environmental justice policy (i.e., 'MA Limited English HH'), experience population-weighted average day-night land surface temperatures are approximately `r round(UHI24hrPop %>% filter(Group == "MA Limited English HH") %>% select(UHI24hrwMean) %>% pull(),1)`°F warmer than the rural background.
...
ma_towns %>% 
  ...
  transmute(`City/Town` = NAME,
            `Day-Night Pct Rank` = round(pctrank_meanAvgLST.mean,0),
            `Day-Night BGs > 80th` = BGs80thDayNight,
            `UHI Pct Rank` = round(pctrank_UHI24avg.mean,0),
            `UHI BGs > 80th` = BGs80thUHI24h,
            `Night Pct Rank` = round(pctrank_meanNightLST.mean,0),
            `Night BGs > 80th` = BGs80thNight) %>% 
  arrange(`City/Town`) %>% 
...
These tables show two types of data - percentile rankings and counts of block groups exceeding the 80th percentile for percentages of priority populations *AND* for heat:

* Percentile rank (**Pct Rank**) of heat for block groups within a given jurisdiction. The percentile rank indicates the percentage of other jurisdiction values that falls below that percentile. For example, a town with Day-Night percentile rank of 89 means that the average Day-Night land surface temperatures of block groups within that municipality is higher than 89% of the average Day-Night average LSTs of other municipalities in the state. 

* The number of Census block groups (**BGs > 80th**) within a jurisdiction that have a high percentage of one or more priority population groups (80th percentile for the state) *AND* which experiences the highest temperatures (80th percentile for the state) for the metric of heat. For example, a town with a value of 2 under the **Day-Night BGs > 80th** column means that there are two Census block groups in that town that have a high percentage of one or more priority population groups (80th percentile for the state) *AND* these people experience Day-Night average land surface temperatures in the 80th percentile (i.e., warmer than 80% of all other municipalities in the state). 