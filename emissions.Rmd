...
Transportation accounts for a significant share of air pollution. These interactive figures identify communities across Massachusetts that are most overburdened and most vulnerable to three transportation-related burdens: PM~2.5~, Diesel Particulate Matter (DPM), and Traffic Volume and Proximity.
...
### PM2.5
```{r mapPM25, fig.align="center", fig.cap="Map of highest PM2.5 concentrations and priority populations by Census block group."}
...
leaflet() %>% 
  ...
  addPolygons(data = ma_blkgrps_sf_PM25, 
              ...
              highlight = highlightOptions(
                ...
                fillOpacity = 0.7,
                bringToFront = TRUE)) %>% 
  addLegend(title = "PM<sub>2.5</sub> (&mu;g/m<sup>3</sup>)", 
            pal = palPM25,
...
### DPM
```{r mapDPM, fig.align="center", fig.cap="Map of highest Diesel Particullate Matter (DPM) concentrations and priority populations by Census block group."}
# create color palette
...
leaflet() %>% 
  ...
  addPolygons(data = ma_blkgrps_sf_DPM, 
              ...
              highlight = highlightOptions(
                ...
                fillOpacity = 0.7,
                bringToFront = TRUE)) %>% 
  addLegend(title = "DPM (&mu;g/m<sup>3</sup>)", 
            pal = palDPM,
...
### Traffic
```{r mapPTRAF, fig.align="center", fig.cap="Map of highest Traffic Proximity and Volume (PTRAF) values and priority populations by Census block group."}
# create color palette
...
leaflet() %>% 
  ...
  addPolygons(data = ma_blkgrps_sf_PTRAF, 
              ...
              highlight = highlightOptions(
                ...
                fillOpacity = 0.7,
                bringToFront = TRUE)) %>% 
  addLegend(title = "PTRAF (AADT/Distance(km))", 
            pal = palPTRAF,
...
DPMpopAvg %>% 
  ...
  hc_xAxis(title = NULL) %>% 
  hc_tooltip(pointFormat = "{point.y}% {series.name}") %>% 
  hc_title(text = "Population-Weighted DPM Exposure")
```
...
ma_towns %>% 
  ...
  transmute(`City/Town` = NAME,
            `PM2.5 Pct Rank` = round(pctrank_PM25_19.mean,0),
            `PM2.5 BGs > 80th` = BGs80thPM25,
            `DPM Pct Rank` = round(pctrank_DSLPM_19.mean,0),
            `DPM BGs > 80th` = BGs80thDPM,
            `PTRAF Pct Rank` = round(pctrank_PTRAF_19.mean,0),
            `PTRAF BGs > 80th` = BGs80thPTRAF) %>% 
  arrange(`City/Town`) %>% 
...
These tables show two types of data:

* Percentile rank (Pct Rank) of mean pollutant concentrations or traffic proximity and volume scores for block groups within a given jurisdiction. The percentile rank indicates the percentage of other jurisdiction values that falls below that percentile. For example, a town with a PM~2.5~ percentile rank of 89 means that the average PM~2.5~ concentrations of block groups within that municipality is higher than 89% of the average PM~2.5~ concentrations of other municipalities in the state. 
* The number of Census block groups (BGs) within a jurisdiction that have a high percentage of one or more priority population groups (80th percentile for the state) *AND* which experiences the highest level of emissions-related burdens (80th percentile for the state) for the respective emissions-related burdens.
